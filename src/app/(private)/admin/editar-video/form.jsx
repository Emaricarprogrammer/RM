  ...
  import { updateVideoData, updateVideoFile} from "@/api/Courses/videos/updateVideo";
...
useEffect(() => {
  ...
  try {
    ...
    const titleChanged = currentValues.title !== videoData.title;
    const descriptionChanged = currentValues.description !== videoData.description;
    const videoFileChanged = currentValues.video_file;

    if (!titleChanged && !descriptionChanged && !videoFileChanged) {
      setShowNoChangesError(true);
      return;
    }
    ...
    // 1. Primeiro atualiza os metadados (título/descrição) se necessário
    if (titleChanged || descriptionChanged) {
      const payload = {};
      if (titleChanged) payload.title = currentValues.title;
      if (descriptionChanged) payload.description = currentValues.description;

      const result = await updateVideoData(videoId, payload, accessToken);
      if (!result.success) {
    ...
    // 2. Depois atualiza o vídeo se necessário
    if (videoFileChanged) {
      const videoResult = await updateVideoFile(videoId, currentValues.video_file, accessToken);
      if (!videoResult.success) {
        throw new Error(videoResult.message);
      }
      setSubmitSuccess(prev => prev 
        ? `${prev}. ${videoResult.message}` 
        : videoResult.message);
    }

    // Redireciona apenas se ambas operações foram bem sucedidas
    router.push(
      `/admin/videos?id=${videoData.id_course_fk}`
    );
  } catch (error) {
    setSubmitError(error.message || "Erro ao atualizar o vídeo");
